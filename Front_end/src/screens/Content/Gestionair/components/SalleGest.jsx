...
const SalleGest = ({nom,type,capacity,chambreid,reset,setReset}) => {
  ...
  return (
    <motion.div className={`py-2 px-3 rounded-md text-sm border-separator border-2 min-w-[230px] hidden ${!amphi && !salle?'!flex':'hidden'} ${salle && type === 'Salle' && !amphi && '!flex'} ${amphi && !salle && type === 'Amphi' && '!flex'}`}>
